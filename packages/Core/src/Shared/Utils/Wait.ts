 ...
 */
export const delay = (timeout:number = 50) => {
 ...
 */
export const waitOnWindowProperty = function (property: string, timeout: number = 30000, waitTime: number = 50) {
    return new Promise(async (resolve, reject) => {
        const startTime = Date.now();
        const message = `property ${property} not found within timeout ${timeout}`;
        while(!window[property]) {
            if (Date.now() - startTime > timeout) {
                window.BOOM.logger.log(message);
                reject(message);
                break;
            }
        ...
        else { window.BOOM.logger.log(message); reject(message) }
    });
}